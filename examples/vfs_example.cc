...
int main() {
    ...
    if (!vfs_result) {
        std::cerr << "Failed to build VFS: " << vfs_result.error().message
                  << std::endl;
        return 1;
    }
    ...
    if (src_id) {
        ...
        if (children) {
            std::cout << "src directory has " << children->size()
                      << " children:" << std::endl;
        ...
        if (entry_file) {
            ...
            if (entry_node) {
                std::cout << "Entry file for src: " << (*entry_node)->name
                          << std::endl;
            }
        }
    }
    ...
    if (main_id) {
        ...
        NodeIndex root_node
            = ast->add_node(NodeBuilder(NodeKind::Int, Span(0, 1)));
