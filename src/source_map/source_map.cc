...
auto SourceMap::get_span_text(const Span& span) const -> std::optional<std::string> {
    ...
    // Find which file(s) contain this span - all of the span must be covered
    std::string result;
    ...
    for (const auto& file : files) {
        ...
        // Check if this file overlaps with the uncovered part of the span
        if (covered_end < file_end && span.end > file.start_pos && covered_end >= file.start_pos) {
            u32 local_start = std::max(covered_end, file.start_pos) - file.start_pos;
...
auto SourceMap::format_span(const Span& span) const -> std::optional<std::string> {
    ...
    if (start_loc->file == end_loc->file && start_loc->line == end_loc->line) {
        ...
        oss << file->name << ":" << start_loc->line << ":" << (start_loc->column + 1) << "-"
            << (end_loc->column + 1);
    } else {
        // Multiple lines
