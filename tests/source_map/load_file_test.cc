...
int main() {
    ...
    if (file1 && file2) {
        std::cout << "✓ Loaded test_source.txt as FileId: " << file1->id << std::endl;
        std::cout << "✓ Loaded fibonacci.bl as FileId: " << file2->id << std::endl;
        ...
        if (f1 && f2) {
            std::cout << "✓ test_source.txt: " << f1->content.size() << " bytes" << std::endl;
            std::cout << "✓ fibonacci.bl: " << f2->content.size() << " bytes" << std::endl;
        }
        ...
        if (fib_pos != std::string::npos) {
            auto global_pos =
                source_map.lookup_byte_pos(Location(*file2, 1, static_cast<u32>(fib_pos)));
            if (global_pos) {
                auto loc = source_map.lookup_location(*global_pos);
                if (loc) {
                    std::cout << "✓ Found 'fibonacci' at: " << source_map.format_location(*loc)
                              << std::endl;
                }
            }
        }
        ...
        auto start_pos = source_map.lookup_byte_pos(Location(*file2, 1, func_start));
        auto end_pos = source_map.lookup_byte_pos(Location(*file2, 1, func_end));

        if (start_pos && end_pos) {
            Span func_name_span(*start_pos, *end_pos);
            auto span_text = source_map.get_span_text(func_name_span);
            if (span_text) {
                std::cout << "✓ Function name: '" << *span_text << "'" << std::endl;
            }
        }
        ...
        if (found_id1 && found_id2 && *found_id1 == *file1 && *found_id2 == *file2) {
