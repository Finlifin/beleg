...
int main() {
    ...
    if (line_content) {
        std::cout << "Line 2 in main.bl: \"" << *line_content << "\""
                  << std::endl;
    }
    ...
    std::cout << "Error at " << source_map.format_location(error_loc) << ":"
              << std::endl;
    ...
    std::cout << "Invalid span (20, 10): valid=" << invalid_span.is_valid()
              << std::endl;
    ...
    if (loaded_file) {
        std::cout << "Successfully loaded test_source.txt "
                     "with FileId: "
                  << loaded_file->id << std::endl;
        ...
        // Test loading the same file again (should return
        // same ID)
        auto loaded_again = source_map.load_file("test_source.txt");
        if (loaded_again && *loaded_again == *loaded_file) {
            std::cout << "Loading same file again returned "
                         "same FileId (caching works!)"
                      << std::endl;
        }
    } else {
        std::cout << "Failed to load test_source.txt (file "
                     "not found)"
                  << std::endl;
    }
